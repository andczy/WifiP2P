...
//		getSharedPreferences("last_share_dir", Context.MODE_PRIVATE).getString("dir", Environment.getExternalStorageState())
...
//		new AP().readEapConfig(out, this);
		mLock = ((PowerManager)getSystemService(Context.POWER_SERVICE)).newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "phone_server") ;
		checkUpdate() ;
		try {
...
//		query.whereEqualTo("playerName", "steve");
	...
	private void showUpdate(Context context , final String msg , final String url){
		...
		sure.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Download.getInstance(WifiAPActivity.this).download(url,R.drawable.ic_launcher,getString(R.string.app_name));
			}
		});
	}
	...
	private  Bitmap getQRBitmap(String qrString, int size) {
		...
		try {
			bitMatrix = multiFormatWriter.encode(qrString,
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				qrBitmap.setPixel(i, j, bitMatrix.get(i, j) ? Color.BLACK
						: Color.WHITE);
			}
		}
		return qrBitmap;
	...
	private void startShare(){
		{
			...
			if(wifiInfo!=null&&wifiInfo.getIpAddress()!=0&&wifiInfo.getNetworkId()!=0){
